 ...
 * four times color b, one time color h then one time color m...
 *
...
function printerError(s) {
    ...
    let numErrors = s.split('').filter(letter => errorLetters.includes(letter)).length;
    let numLetters = s.length;
    let errorRate = `${numErrors}/${numLetters}`;
    return errorRate;
}

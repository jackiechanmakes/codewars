...
function comp(arrA, arrB) {
    arrB = arrB ? arrB.map(el => Math.sqrt(el)).sort((a, b) => a - b) : null;
    arrA ? arrA.sort((a, b) => a - b) : null;
    return arrA && arrB && arrA.length === arrB.length ? arrB.every((el, ind) => el === arrA[ind]) : false;
}

// Test cases
console.log(comp([121, 144, 19, 161, 19, 144, 19, 11], [11*11, 121*121, 144*144, 19*19, 161*161, 19*19, 144*144, 19*19])); // true
console.log(comp([132, 144, 19, 161, 19, 144, 19, 11], [11*11, 121*121, 144*144, 19*19, 161*161, 19*19, 144*144, 19*19])); // false
